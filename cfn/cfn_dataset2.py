...
class cfn_lerobot_dataset2(LeRobotDataset):
    def __init__(
        self,
        repo_id: str,
        ...
        self.processor.image_processor.do_convert_rgb = False
        # import ipdb; ipdb.set_trace()
    ...
    def __getitem__(self, idx) -> dict:
        ...
        smol_inputs = self.extract_smol_inputs(images_list, item["task"])
        smol_inputs['input_ids'], smol_inputs['attention_mask'] = self.pad_input_ids(
            max_length=250, 
            input_ids=smol_inputs['input_ids'],
            attention_mask=smol_inputs['attention_mask'],
        )
        smol_inputs = {key: value.squeeze(0) for key, value in smol_inputs.items()}

        item["smol_inputs"] = smol_inputs
