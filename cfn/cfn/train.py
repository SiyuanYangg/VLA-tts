...
from cfn_dataset import cfn_lerobot_dataset
...
def make_dataset(cfg: TrainPipelineConfig) -> cfn_lerobot_dataset | MultiLeRobotDataset:
    ...
    if isinstance(cfg.dataset.repo_id, str):
        ...
        # 按比例划分 train_rat 训练，1-train_rat 测试
        train_rat = 0.999
...
def evaluate(model, val_dataset, device, cfn_action_steps):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            loss, model_output_val = model.compute_loss(state, action, instructions, target)
...
def train(cfg: TrainPipelineConfig):
    ...
    for epoch in range(total_epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch + 1}"):
            ...
            loss, model_output_train = model.compute_loss(state, action, instructions, target)
