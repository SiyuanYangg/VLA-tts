...
class CoinFlipMaker:
    ...
    def __call__(self, seed=None, batch_size=1):
        ...
        flips = rng.binomial(1, 0.5, size=(batch_size, self.output_dimensions))
        return 2 * flips - 1
...
# ========== CFN 包装类（支持图文编码） ========== #
class CFNWrapper_pi_prior_big_noised_action(nn.Module):
    def __init__(self,
                 cfn_output_dim=64,
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            actions, denoising_feature = self.policy.get_denoising_feature(batch)
            denoising_feature = denoising_feature.to(next(self.parameters()).dtype)
        # import ipdb; ipdb.set_trace()
        denoise_step, batch_size, feature_dim = denoising_feature.shape
        assert denoise_step == 10
        denoising_feature = denoising_feature.reshape(denoise_step*batch_size, feature_dim)
        return self.cfn(denoising_feature)
    
    def compute_loss(self, batch):
        preds = self.forward(batch)
        self.cfn.update_prior()
        batch_size = batch['action'].shape[0]
        targets = torch.from_numpy(self.maker(batch_size=10*batch_size)).to(dtype=torch.float32).to(next(self.parameters()).device)
