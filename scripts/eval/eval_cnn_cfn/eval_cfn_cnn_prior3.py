...
def test(task, tag):
    ...
    eval_task = ckpt_task
    data42 = torch.load(
        f"/gemini/platform/public/embodiedAI/users/ysy/data/robotwin/dataset/rt-record-traj/noised_action/noise_seed42/{eval_task}/traj_0.pt",
        weights_only=False,
    )
    data2345 = torch.load(
    ...
    for i in range(data42["noised_action"].shape[0]):
        norm42 = {}
        norm2345 = {}
        infer_step = data42["noised_action"].shape[1]
        # import ipdb; ipdb.set_trace()
        # ======== forward seed42 ========
        batch42 = {
            "action": data42["noised_action"][i, ...],
            "observation.state": data42["state"][[i], :].repeat(infer_step, 1),
            "observation.images.cam_high": data42["img_high"][[i], ...].repeat(infer_step, 1, 1, 1),
            "observation.images.cam_left_wrist": data42["img_left"][[i], ...].repeat(infer_step, 1, 1, 1),
            "observation.images.cam_right_wrist": data42["img_right"][[i], ...].repeat(infer_step, 1, 1, 1),
        }
        ...
        # ======== 保存 json ========
        # os.makedirs(f"{tag}/noised_action/{eval_task}/seed42", exist_ok=True)
        # with open(f"{tag}/noised_action/{eval_task}/seed42/norm_infer_{i}.json", "w", encoding="utf-8") as f:
        #     json.dump(norm42, f, ensure_ascii=False, indent=4)
        # print(f"save at {tag}/noised_action/{eval_task}/seed42/norm_infer_{i}.json")

        # os.makedirs(f"{tag}/noised_action/{eval_task}/seed2345", exist_ok=True)
        # with open(f"{tag}/noised_action/{eval_task}/seed2345/norm_infer_{i}.json", "w", encoding="utf-8") as f:
        #     json.dump(norm2345, f, ensure_ascii=False, indent=4)
        # print(f"save at {tag}/noised_action/{eval_task}/seed2345/norm_infer_{i}.json")
        ...
        os.makedirs(f"{tag}/noised_action/{eval_task}/plots", exist_ok=True)
        plt.savefig(f"{tag}/noised_action/{eval_task}/plots/norm_traj_{i}.png", dpi=150)
        plt.close()
