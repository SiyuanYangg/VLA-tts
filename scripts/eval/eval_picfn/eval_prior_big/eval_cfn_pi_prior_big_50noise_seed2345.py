...
def test(task, tag):
    ...
    eval_task = ckpt_task
    data42 = torch.load(
        f"/gemini/platform/public/embodiedAI/users/ysy/data/robotwin/dataset/rt-record-traj/noised_action_50noise/noise_seed2345/{eval_task}/traj_{traj_num}.pt",
        weights_only=False,
    )
    # data2345 = torch.load(
    ...
    for i in range(batch_size):
        ...
        os.makedirs(f"{tag}/noised_action_50noise/{eval_task}/seed2345/traj{traj_num}_is_suc{norm42['is_suc']}/plots", exist_ok=True)
        plt.savefig(f"{tag}/noised_action_50noise/{eval_task}/seed2345/traj{traj_num}_is_suc{norm42['is_suc']}/plots/norm_infer_{i}.png", dpi=150)
        plt.close()
