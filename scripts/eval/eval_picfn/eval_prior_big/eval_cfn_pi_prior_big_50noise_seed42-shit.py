import torch
from cfn.pi0_cfn.cfn_net_pi_prior_big2 import CFNWrapper_pi_prior_big
import json
import os
...
def test(task, tag):
    ...
    weight_path = f"/gemini/platform/public/embodiedAI/users/ysy/data/train_cfn/{tag}/{ckpt_task}-0908/model_epoch8.pt"
    ...
    for i in range(batch_size):
        ...
        actions, feature = model.policy.select_action_and_get_feature(batch42, noise.clone())
        feature = feature.to(next(model.cfn.parameters()).dtype)
        import ipdb;ipdb.set_trace()
...
if __name__ == "__main__":
    ...
    for task in ["block_handover"]:
        ...
        test(task, "cfn_pi-single_task-newckpt-prior-big-noised-action-0908")
